...
export class NominatimGeocoder {
    ...
    buildUrl(latitude, longitude) {
        return buildNominatimReverseUrl(latitude, longitude, this.baseUrl, this.corsProxy);
    }
    async reverseGeocode(latitude, longitude) {
        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
            throw createGeoReverseGeocodeError(1, '(NominatimGeocoder) Invalid coordinates');
        }
        try {
    ...
    async fetchAndMap(latitude, longitude) {
        ...
        return toGeoAddressFromNominatim(raw);
    }
    async fetchRaw(url) {
        if (this.legacyFetchManager) {
            try {
                ...
                throw createGeoReverseGeocodeError(2, `(NominatimGeocoder) ${message}`);
            }
        }
        let response;
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            throw createGeoReverseGeocodeError(2, `(NominatimGeocoder) Network error: ${message}`);
        }
        if (!response.ok) {
            throw createGeoReverseGeocodeError(3, `(NominatimGeocoder) HTTP ${response.status}: ${response.statusText}`);
        }
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            throw createGeoReverseGeocodeError(3, `(NominatimGeocoder) Invalid JSON: ${message}`);
        }
    }
